...
answer_1=$(zcat $states | cut -f1,6 | sort -k2n | grep -v '^#' | head -n1 | cut -f1 | sed 's/"//g' | sed 's/_/ /g')
...
answer_2=$(cat $samples | grep "^>" | wc -l)
...
answer_3=$(zcat $cpg | cut -f4 | sort -g | uniq | wc -l)
...
answer_5=$(cat $hamlet | grep 'bloody' | wc -w)

echo "answer-5: $answer_5"

# What is the length of the sequence in the first record of sample.fa? 

answer_6=$(head -n2 $samples | grep -v '^>' | wc -c)
...
answer_7=$(zcat $genes | awk 'BEGIN {OFS="\t"}{print $0,
$3-$2}'| sort -k7nr |head -n1 | awk '{print $4}')

echo "answer-7: $answer_7"

# How many unique chromosomes are in genes.hg19.bed.gz? 

answer_8=$(zcat $genes | awk '{print $1}' | sort -u | wc -l)
...
answer_9=$(zcat $peaks | awk '{print $4}' | sort | uniq -c | grep -w 'CTCF' | awk '{print $1}')
...
answer_10=$(cat $lamina | awk 'BEGIN {OFS="\t"}{print $0, $3-$2}' |
sort -k5nr | head -n1 | awk '{print $1}')
